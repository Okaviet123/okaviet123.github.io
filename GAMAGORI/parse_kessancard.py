...
# -*- coding: utf-8 -*-
"""
parse_kessancard.py — 決算カードPDF（総務省の全国統一様式）から
財政指標の時系列CSVを作る汎用パーサー。豊橋市#1で開発したものを
市名非依存に一般化した `SKILLS/muni-facility-report/scripts/parse_kessancard.py`
を蒲郡市向けにそのまま採用したもの（決算カードは総務省の統一フォーマット
のため、原則コード変更不要）。

入力 : RAW/kessancard/ 配下の *.pdf（ファイル名は自由。中身から年度を判定する）
出力 : DATA/kessancard_timeseries.csv
方式 : pdftotext -layout（poppler-utils）を subprocess で呼び、テキストを正規表現で解析。
       ※ CJK非埋め込みフォントのPDFは poppler-data パッケージが
         ないと空テキストになる。要 `apt-get install poppler-data`。

使い方: 対象市のRAW/kessancard/にPDFを置いてから
  python3 parse_kessancard.py
を実行する。1年度分だけでまず様式を確認してから全件処理するのが安全
（年度によって実質公債費比率・将来負担比率の有無等、様式が変わる自治体もある）。

方針 :
- ラベルは文字間の空白ゆらぎ（「実 質 公 債費 比率」等）に対応するため、
  各文字の間に \s* を挟んだ正規表現で照合する。
- 取れない値は空欄。推測で埋めない。「その年度の様式に存在しない項目」と
  「抽出ミス」は別物 — 前者は正当な空欄として扱い、混同しない。
- △ は負数。― / − / – は欠損（空欄）扱い。
...
# ファイル名からの推定はフォールバック用（市によって命名規則が違うため
# 中身のノンブル・見出しから年度を読むのを優先する）
YEAR_IN_FILENAME_RE = re.compile(r"([HRS])(\d+|元)")


def _year_tuple(era_kanji_or_code: str, n_str: str):
    n = 1 if n_str == "元" else int(n_str)
    if era_kanji_or_code in ERA_KANJI:
        era = ERA_KANJI[era_kanji_or_code]
    else:
        era = era_kanji_or_code
    wareki = f"{ERA_NAME[era]}{'元' if n == 1 else n}年度"
    return ERA_BASE[era] + n, wareki, era, n


def year_from_content(lines):
    """カード冒頭（表題部分）の「令和６年度」等から年度を判定する。
    決算カードはこの表記が1ページ目の左上に必ず出る。"""
    for ln in lines[:15]:
...
def year_from_filename(fname: str):
    m = YEAR_IN_FILENAME_RE.search(fname)
    if not m:
        return None
    return _year_tuple(m.group(1), m.group(2))
...
def parse_one(pdf_path: str):
    ...
    year_info = year_from_content(lines) or year_from_filename(fname)
    if year_info is None:
        raise RuntimeError("年度を特定できません（中身にもファイル名にも令和/平成/昭和+年度の表記が見当たらない）")
...
def main():
    ...
    if not files:
        print(f"{RAW_DIR} にPDFが見つかりません。決算カードを置いてから実行してください。")
        return
