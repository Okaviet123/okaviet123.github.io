...
注意: DISTRICT_COL / SPEND_COL / USERS_COL は、公共施設白書の個別票を
実際に読んでから確定させること（下記はS1偵察時点の暫定値。中学校区単位で
地区別個別計画が組まれているとの情報に基づく仮置きで、白書内の実表記が
「小学校区」等であれば修正する）。
"""
import csv
...
# ==== ★蒲郡市向けに書き換え（暫定値。白書取得後に確定させること） ====
ROOT = Path(__file__).parent
BASE_YEAR = 2026        # 築年数の基準年（レポート公開年に合わせる。公開日未定のため暫定）
OLD_THRESHOLD = 40      # 「築古」とみなす年数の下限
TOP_N = 50              # 築古×延床上位テーブルの件数
DISTRICT_COL = "中学校区"  # 暫定。白書の個別票の実表記に合わせて修正すること（S1_recon_matrix.md参照）
SPEND_COL = "支出_円"       # 直近年度の支出額列。年度が確定したら「支出20XX_円」に変更
USERS_COL = "利用者数_人"   # 直近年度の利用者数列。年度が確定したら「利用者数20XX_人」に変更
# ================================
