...
CONFIG = {
    ...
    "materials": [
        ("Ａ", "蒲郡市公共施設マネジメント実施計画（本編）(平成29年3月)", "14〜15ページ"),
        ("Ｂ", "蒲郡市公共施設白書（令和2年度改訂版、令和3年3月）", "各施設の該当ページ（表内に記載）"),
    ],
    "source_urls": [
        "資料Ａ: https://www.city.gamagori.lg.jp/uploaded/attachment/41055.pdf",
    ...
    "terms": [
        ...
        ("建物系施設", "市民館や学校など「建物」の施設。道路・橋・水道（インフラ）や"
                        "上下水道・競艇場は、この試算には入っていません。"),
        ("ライフサイクルコスト試算", "建物を今のまま使い続けた場合、これから何十年で"
                        "維持・更新にいくらかかるかを見積もった数字。"),
        ("老朽化度", "白書が独自に算出した、建物の傷み具合を表す指数（数字が大きいほど"
                        "老朽化が進んでいる）。"),
    ],
    # 柱になる数字: (項目文, 出どころ, チェック選択肢)
    "pillars": [
        ("蒲郡市の建物系公共施設をこれから30年維持・更新するのに必要なお金は "
         "<b>約1,081億円</b>。<br>これは、白書のライフサイクルコスト試算（平成27年からの"
         "50年間で1,801億円、年平均約36.0億円）を、市の計画期間である30年に換算した"
         "当方の計算です（市が発表している523億円の目標を算出したのと同じやり方で"
         "換算しています。詳しくは下の「当方の集計」欄をご覧ください）。",
         "資料Ａ 14ページ図表1-14", "資料と一致していた/違っていた/わからない"),
        ("一方、市が用意できる見込みは <b>約558億円</b>（直近の実績支出・年平均約18.6億円を"
         "30年続けた場合）。",
         "資料Ａ 14ページ", "資料と一致していた/違っていた/わからない"),
        ("つまり <b>約523億円（約48%）足りない</b>、と市自身が試算している"
         "（この523億円は、市が自ら「871億円÷50年×30年」で30年に換算して発表した数値です）。",
         "資料Ａ 15ページ", "資料と一致していた/違っていた/わからない"),
        ("市の目標は「<b>建物の更新の際に概ね3割（約30%）の床面積を縮減する</b>」こと。",
         "資料Ａ 15ページ「＜計画期間における目標＞①」",
         "資料と一致していた/違っていた/わからない"),
    ],
    # 「当方の集計」の項目（市の発表ではない独自計算）
    "own_aggregation": {
        "claim": "白書に個別データがある109施設のうち、建築年度が公表されている109施設"
                 "（全施設）を集計すると、<b>延べ床面積の79%が築40年以上</b>（2026年時点）。",
        "method_lines": [
            "元の数字: 白書第3章（施設用途ごとの老朽化状況・利用状況・コスト状況の表）と"
            "第4章（地区別の施設一覧表）から、109施設ぶんの施設名・延べ床面積・建築年度・"
            "地区・利用者数・支出額を、該当ページとあわせて全部書き写しました。",
            "計算: 建築年度が2026年より40年以上前（＝築40年以上）の施設の延べ床面積を合計し、"
            "全109施設の延べ床面積合計（約29.1万㎡）で割りました。",
        ],
        "verify_note": "書き写した一覧表（全109施設について出典ページを明記したもの。"
                        "リポジトリに DATA/shisetsu_list.csv として公開しています）を"
                        "一緒にお渡しします。お好きな施設を2〜3件選んで、白書の該当ページと"
                        "一覧表の行が一致するか確認してください。土台の書き写しが正しければ、"
                        "合計は機械的な足し算です。",
    },
    # よく知られた施設: (施設名, 地域単位, 建設年度, 広さ, 支出, 利用者, 出典記号+ページ)
    "known_facilities": [
        ("本庁舎", "蒲郡北", "1961年", "19,817㎡", "約2.5億円", "（把握できず）", "Ｂ p.30"),
        ("竹島水族館", "蒲郡南", "1962年", "1,996㎡", "約0.7億円", "365,563人", "Ｂ p.84"),
        ("生命の海科学館", "蒲郡南", "1998年", "3,297㎡", "約1.1億円", "84,785人", "Ｂ p.40"),
        ("蒲郡中学校", "蒲郡南", "1960年", "9,383㎡", "約0.5億円", "489人（生徒数）", "Ｂ p.53"),
        ("蒲郡公民館", "蒲郡南", "2013年", "806㎡", "約0.1億円", "17,696人", "Ｂ p.34"),
        ("学校給食センター", "三谷", "2004年", "3,692㎡", "約6.7億円", "1,443,801食（配食数）", "Ｂ p.57"),
        ("クリーンセンター", "西浦", "1997年", "7,139㎡", "約7.3億円", "（把握できず）", "Ｂ p.77"),
    ],
    "district_label": "地区",
}
# ============================
...
<p class="lead" style="font-size:9.5pt">
「支出」は令和2年度改訂の白書が掲載する平成26〜令和元年度の6ヵ年度平均額（単年度の実額ではありません）。
「利用者」は施設によって数え方が違います（来館者数・生徒数・給食の配食数など）。
白書がそもそも人数を把握していない施設（本庁舎・クリーンセンター等）は「（把握できず）」としています。</p>
